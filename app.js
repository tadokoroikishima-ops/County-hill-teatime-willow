...
function applyGallery(srcs) {
  ...
  const list = srcs.length > 0
    ? srcs.map((src, i) => ({ src, label: `Work ${String(i + 1).padStart(2, '0')}` }))
    : Array.from({ length: 6 }, (_, i) => ({ src: '', label: `Work ${String(i + 1).padStart(2, '0')}` }));
  ...
  function createCard(work, index) {
    ...
    if (work.src) {
      ...
      ph.innerHTML = `<span class="material-icons">add_photo_alternate</span>${work.label}`;
      card.appendChild(ph);
    }
